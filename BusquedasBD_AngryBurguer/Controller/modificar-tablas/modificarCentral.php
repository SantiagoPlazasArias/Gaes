...
//conexion BD
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $actualizar = $miPDO->prepare('UPDATE central SET Nombre=:Nombre, Direccion=:Direccion WHERE id=:id');
        $actualizar->execute([':id' => $id, ':Nombre' => $Nombre, ':Direccion' => $Direccion, ]);
        header('Location: ../../View/indexform.php');
    }
    else{
        $consultar=$miPDO->prepare('SELECT * FROM central  WHERE id=:id');
