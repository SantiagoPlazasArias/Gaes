...
$Cobertura=isset($_REQUEST['Cobertura'])?$_REQUEST['Cobertura']:null;
...
//conexion BD
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $actualizar = $miPDO->prepare('UPDATE domicilios SET Nombre=:Nombre, Documento_Identidad=:Documento_Identidad, , Telefono=:Telefono, Correo=:Correo, Direccion=:Direccion, Cobertura=:Cobertura WHERE id=:id');
        $actualizar->execute([':id' => $id, ':Nombre' => $Nombre, ':Documento_Identidad' => $Documento_Identidad, ':Telefono' => $Telefono, ':Correo' => $Correo, ':Direccion' => $Direccion, ':Cobertura'=>$Cobertura ]);
        header('Location: ../../View/indexform.php');
    }
    else{
        $consultar=$miPDO->prepare('SELECT * FROM domicilios  WHERE id=:id');
...
<head>
    ...
    <title>modificar Domicilios</title>
...
<li>
        ...
        <label for="Direccion">Cobertura:</label>
        <input id="Direccion" name="Cobertura" type="text" value="<?=$usuario['Cobertura']?>">
